...
window.fillRestaurantHTML = (restaurant = self.restaurant) => {
  ...
  image.src = DBHelper.mdImageUrlForRestaurant(restaurant);
...
reviewsForm.addEventListener("submit", e => {
  ...
  const restaurantId = getParameterByName("id", window.location);
  const name = reviewsForm.elements["firstName"].value;
  const rating = reviewRating.getRating();
  ...
  DBHelper.storeReview(newReview, review => {
    ul.appendChild(createReviewHTML(review));
    reviewsForm.reset();
  });
});
